...
class Crawler():
    ...
    def _clean_row(self, row):
        ...
        for index, content in enumerate(row):
            ...
            row[index] = re.sub("---", "0", content.strip())
        return row

    def _record(self, stock_id, row):
        ''' Save row to csv file '''
        file_name = '{}/{}.csv'.format(self.prefix, stock_id)
        self._csv_init(file_name)
        f = open(file_name, 'a')
        cw = csv.writer(f, lineterminator='\n')
        cw.writerow(row)
        f.close()

    def _csv_init(self, file_name):
        init_data = ['date', 'volume', 'value_NTD', 'open',
                     'high', 'low', 'close', 'change', 'transac']
        if not os.path.isfile(file_name):
            f = open(file_name, 'a')
            cw = csv.writer(f, lineterminator='\n')
            cw.writerow(init_data)
            f.close()
    ...
    def _get_tse_data(self, date_tuple):
        ...
        date_str_mingguo = '{0}/{1:02d}/{2:02d}'.format(
...
def main():
    ...
    if args.back or args.check:
        ...
        last_day = datetime(
            2004, 2, 11) if args.back else first_day - timedelta(365)
        max_error = 5
        error_times = 0
        ...
        while error_times < max_error and first_day >= last_day:
            try:
                if first_day.weekday() <= 4:
                    ...
                    print('{} is weekday'.format(first_day))
                error_times = 0
            except:
